"""Unit tests for the shrimp eval statistics layer (pure numpy/CPU).

Covers the functions in ``packages/shrimp/python/shrimp/eval_stats.py``:
...
shrimp is not installed in a shared venv; its source is added to ``sys.path``
...
from shrimp.eval_stats import (  # noqa: E402
    ELO_PER_LOGIT,
    BTEdge,
    bonferroni_alpha,
    bradley_terry,

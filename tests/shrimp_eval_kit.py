"""Shared test fixtures for the shrimp eval suites (CPU-only, no GPU/torch/.so).

Contents:

  * Engine/session fakes (used by ``test_shrimp_eval_arena``):
  ...
  * Orchestrator stubs (used by ``test_shrimp_eval_orchestrator`` and
...
Importing this module touches only ``shrimp.geometry`` and
...
from shrimp.geometry import pack_action_id  # noqa: E402
...
def _make_run(tmp_path: Path, epochs=(5, 10, 20, 40), *, bc: bool = True) -> Path:
    ...
    ``<run>/checkpoints`` and the BC prefit in a sibling ``shrimp_bc_1``.
    ...
    if bc:
        bc_dir = tmp_path / "runs" / "shrimp_bc_1"
...
class _StubArena:
    ...
    def play_sealbot_match(self, ckpt, n, **kw) -> dict:
        ...
        return _sealbot_match(kw.get("label", "shrimp"), n, self._sealbot_winrate)
...
class _FakeArena:
    ...
    def play_sealbot_match(self, ckpt, n, **kw) -> dict:
        ...
        match = _sealbot_match(kw.get("label", "shrimp"), n, self._sealbot_winrate)

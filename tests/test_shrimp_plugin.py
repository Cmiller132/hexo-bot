...
from shrimp_testkit import api
from hexo_engine.types import AxialCoord, PlacementAction

from shrimp.checkpoints import ShrimpCheckpointSaver, load_into, save_checkpoint
...
from shrimp.plugin import get_plugin
...
from shrimp.window import load_packed_shard
...
def test_config_rejects_unknown_keys() -> None:
    with pytest.raises(ValueError, match="unknown"):
        parse_shrimp_config({"selfplay": {"not_a_key": 1}})


def test_config_rejects_unknown_top_level_keys() -> None:
    # An unknown top-level section key raises rather than being ignored.
    with pytest.raises(ValueError, match="unknown ShrimpConfig keys"):
        parse_shrimp_config({"slfplay": {"search_visits": 256}})
...
def test_config_override() -> None:
    cfg = parse_shrimp_config({"selfplay": {"search_visits": 256, "search_parity_mode": True}})
    assert cfg.selfplay.search_visits == 256
...
def test_pcr_fast_temperature_round_trips_and_defaults_off() -> None:
    ...
    assert parse_shrimp_config({}).selfplay.pcr_fast_temperature == 0.0
    # Explicit value round-trips through the [selfplay] parser.
    cfg = parse_shrimp_config({"selfplay": {"pcr_fast_temperature": 0.1}})
...
def test_checkpoint_strict_roundtrip(tmp_path) -> None:
    ...
    assert payload["meta"]["lineage"] == "shrimp"
...
def test_checkpoint_strict_load_rejects_mismatch() -> None:
    ...
    with pytest.raises(ValueError, match="key mismatch"):
        load_into(ShrimpNet(), {"model": state, "meta": {}})


def test_plugin_builds_model_and_optimizer_split() -> None:
    plugin = get_plugin()
    assert plugin.name == "shrimp"
...
def _decisive_game_shard(out_dir: Path) -> int:
    """Write one shrimp_compact_v1 shard from a finished game (P0 builds six on
...
def test_trainer_runs_on_real_rows(tmp_path) -> None:
    ...
    model = ShrimpNet()
    cfg = parse_shrimp_config({"device": "cpu", "training": {"batch_rows": 8}})
    opt = torch.optim.AdamW(model.parameters(), lr=1e-3)
    trainer = ShrimpTrainer(model=model, config=cfg, optimizer=opt)

"""Continuous-scheduler self-parity for the shrimp session, plus an
...
restructured to a shrimp-vs-shrimp parity net on the public native module.
...
- an end-to-end payload -> ShrimpNet -> reply -> tree round-trip completes with
  divergences ON.
"""
...
from test_shrimp_search_parity import ShrimpStub
...
class Recorder:
    ...
    def __call__(self, game_key: int, payload: dict):
        from test_shrimp_search_parity import _fully_in_crop
...
def test_divergences_off_equals_parity_mode() -> None:
    ...
    session_a = shrimp_rust.ShrimpMctsSession(max_states=65536)
    session_a.run_continuous(
        keys, tuple(rec_a.states[k] for k in keys), evaluator=ShrimpStub(),
        on_move=rec_a, search_parity_mode=True, **kwargs,
    )
    session_b = shrimp_rust.ShrimpMctsSession(max_states=65536)
    session_b.run_continuous(
        keys, tuple(rec_b.states[k] for k in keys), evaluator=ShrimpStub(),
        on_move=rec_b,
...
def test_e2e_abi_roundtrip_with_real_model() -> None:
    """Payload -> packer -> ShrimpNet -> reply -> tree with divergences ON,
    ...
    torch.manual_seed(0)
    evaluator = ShrimpEvaluator(ShrimpNet(), device="cpu")
    session = shrimp_rust.ShrimpMctsSession(max_states=4096)

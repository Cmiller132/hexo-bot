"""Stub-evaluator self-parity for the hexfield MCTS session.

Originally a differential harness against the (now retired) dense_cnn session;
restructured to a hexfield-vs-hexfield parity net that runs entirely on the
...
within a radius-20 crop of the stones' rounded centroid, so the ``HexfieldStub``

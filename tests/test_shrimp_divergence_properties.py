...
from test_shrimp_search_parity import ShrimpStub, _corpus
...
def test_lcb_matches_closed_form_tables() -> None:
    ...
    for _ in range(300):
        ...
        got = shrimp_rust.debug_lcb_pick(stats, z, minv, frac)
...
def test_lcb_eligibility_fallback() -> None:
    ...
    assert shrimp_rust.debug_lcb_pick(stats, 1.6, 8, 0.1) is None


@needs_native
def test_ml_bonus_properties() -> None:
    w, scale, gate = 0.03, 32.0, 0.6
    bonus = lambda q, me, mn: shrimp_rust.debug_ml_bonus(q, me, mn, w, scale, gate)
...
def test_ml_bonus_two_sided() -> None:
    w, scale, gate = 0.03, 32.0, 0.6
    one = lambda q, me, mn: shrimp_rust.debug_ml_bonus(q, me, mn, w, scale, gate, False)
    two = lambda q, me, mn: shrimp_rust.debug_ml_bonus(q, me, mn, w, scale, gate, True)
...
def test_gumbel_target_c_scale_and_draw_temperature_toml_round_trip() -> None:
    ...
    default_sp = parse_shrimp_config({}).selfplay
    ...
    sp = parse_shrimp_config(cfg).selfplay
...
def test_early_stop_without_lcb_is_exact() -> None:
    ...
    states = _corpus(60)
    on = shrimp_rust.ShrimpMctsSession(max_states=65536)
    off = shrimp_rust.ShrimpMctsSession(max_states=65536)
    overrides_on = {
    ...
    for index, state in enumerate(states):
        kwargs = dict(
            ...
            evaluator=ShrimpStub(), virtual_batch_size=8,
...
def test_reused_root_temperature_applied_once() -> None:
    ...
    payload_rows = shrimp_rust.featurize_states([state])
    row = payload_rows[0]

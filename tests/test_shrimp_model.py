...
from shrimp.geometry import rel_bias_index
from shrimp.model import ShrimpNet, _BiasGather

# Sum of p.numel() over a fresh ShrimpNet() (8 conv + 3 attn blocks,
...
def _derandomize(model: ShrimpNet, seed: int = 5) -> None:
...
def test_conv_oracle_against_dense_grid_conv() -> None:
    ...
    axial-to-offset embedding used throughout shrimp: tap 0 is the centre tap
    ...
    from shrimp.model import HexNodeConv
...
def test_per_row_grad_accumulation_equals_monolithic() -> None:
    ...
    def row_loss(model: ShrimpNet, batch) -> torch.Tensor:
...
def test_fresh_model_zero_init_residual_identity() -> None:
    """A fresh ShrimpNet (not _derandomized) has LayerScale gamma == 1e-4 on
...
def test_sdpa_equals_materialized_fp16_cuda() -> None:
    ...
    torch.manual_seed(0)
    model = ShrimpNet().eval().to(device)

"""Unit tests for the shrimp multi-stage eval orchestrator (CPU-only, no GPU).

Exercises ``packages/shrimp/python/shrimp/multistage_eval.py``, the layer
that wires the game-running arena to the statistics core and emits a verdict
label. The statistics themselves are covered in ``test_shrimp_eval_stats.py``.
...
``_FakeArena`` / ``_StubArena`` from ``shrimp_eval_kit``), so this collects and
...
sys.path.insert(0, str(_REPO / "packages" / "shrimp" / "python"))

from shrimp import multistage_eval as mse  # noqa: E402
from shrimp.config import (  # noqa: E402
    MultiStageEvalSection,
    parse_shrimp_config,
)

from shrimp_eval_kit import (  # noqa: E402
    _FakeArena,
    _StubArena,
    _make_run,
    _scores_for_winrate,
...
def _no_sprt_config(**overrides):
    ...
    return parse_shrimp_config({"multi_stage_eval": {"sprt": {"enabled": False}, **overrides}})
    ...
    cfg = MultiStageEvalSection(
        opponents=dataclasses.replace(
            ...
            permanent_anchors=(
                ...
                ("bc_prefit", "runs/__shrimp_no_such_bc__/checkpoint_epoch2.pt"),
...
def test_bc_prefit_resolves_from_repo_tree(tmp_path: Path, monkeypatch) -> None:
    """bc_prefit path resolution: the run-data tree has a ``shrimp_bc_1/`` dir
    ...
    # Run-data tree with a shrimp_bc_1 DIR but no BC FILE.
    data_tree = tmp_path / "run-data"
    run = data_tree / "runs" / "shrimp_main_1"
    (run / "checkpoints").mkdir(parents=True)
    (data_tree / "runs" / "shrimp_bc_1").mkdir(parents=True)  # dir only, no file
    ...
    bc_file = repo_tree / "runs" / "shrimp_bc_1" / "checkpoint_epoch2.pt"
    ...
    fake_module_file = repo_tree / "packages" / "shrimp" / "python" / "shrimp" / "multistage_eval.py"
    ...
    resolved = mse._resolve_anchor_path(
        run, run / "checkpoints", "runs/shrimp_bc_1/checkpoint_epoch2.pt"
    )
...
def test_bc_prefit_prefers_run_data_tree_when_present(tmp_path: Path) -> None:
    ...
    assert resolved == tmp_path / "runs" / "shrimp_bc_1" / "checkpoint_epoch2.pt"
    assert resolved.is_file()
    roster = mse.select_opponents(
        run, run / "checkpoints" / "epoch_000040.pt", MultiStageEvalSection(), candidate_epoch=40
    )
...
def test_sprt_triage_maps_accept_h1_to_regress_and_h0_to_ok(tmp_path: Path, monkeypatch) -> None:
    ...
    cfg = parse_shrimp_config({"multi_stage_eval": {"sprt": {"enabled": True, "max_games": 8}}})

    from shrimp import eval_stats
...
def test_full_sims_threaded_into_stage_b_and_c_by_default(tmp_path: Path) -> None:
    ...
    run = _make_run(tmp_path, epochs=(5, 10, 20, 40))
    arena = _FakeArena(ckpt_scorer=lambda lb, n: _scores_for_winrate(0.75, n), sealbot_winrate=0.7)
    cfg = parse_shrimp_config({"multi_stage_eval": {"sprt": {"enabled": True, "max_games": 16}}})
...
def test_changing_the_verdict_touches_no_run_state(tmp_path: Path) -> None:
    ...
    eval_only = {
        ...
        Path("diagnostics") / "shrimp.multistage_eval.epoch_000040.json",
    }
...
def test_sealbot_edge_candidate_searches_as_trained_profile(tmp_path) -> None:
    ...
    from shrimp_eval_kit import _sealbot_match

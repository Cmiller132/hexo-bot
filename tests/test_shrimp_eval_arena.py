...
for _p in ("hexo_engine/python", "shrimp/python"):
...
from shrimp import eval_arena  # noqa: E402  (torch-free import)
...
def _engine_bridge_available() -> bool:
    ...
    Requires the shrimp Rust extension to be importable and the hexo_engine
...
needs_native = pytest.mark.skipif(
    ...
    reason="shrimp/hexo_engine native modules not built (real-ABI tests skipped)",
)
...
def _run_match(monkeypatch, *, n_games, a_strength, b_strength, game_len=6,
    ...
    result = eval_arena.play_checkpoint_match(
        ...
        config=config if config is not None else parse_shrimp_config({}),
...
def test_result_consumed_by_orchestrator_helpers(monkeypatch):
    ...
    from shrimp import multistage_eval as mse
    ...
    import shrimp.eval_stats as es
...
def test_max_plies_truncation_marks_games_undecided(monkeypatch):
    ...
    cfg = parse_shrimp_config({"selfplay": {"max_game_plies": 3}})
...
def test_terminal_and_truncation_coexist_in_one_match(monkeypatch):
    ...
    cfg = parse_shrimp_config({"selfplay": {"max_game_plies": 4}})
...
def _serial_reference(cfg, eval_a, eval_b, *, n_games, opening_plies,
    ...
    for pair_index in range(n_pairs):
        ...
        for a_is_p0 in (True, False):
            ...
            s_a = shrimp_rust.ShrimpMctsSession(max_states=4096)
            s_b = shrimp_rust.ShrimpMctsSession(max_states=4096)
            state = api.new_game()
            line: list[int] = []
            ply = 0
            winner = None
...
def _opening_led_by(cfg, p0_eval, p1_eval, *, seed_base, opening_plies):
    ...
    state = api.new_game()
    line: list[int] = []
    s0 = shrimp_rust.ShrimpMctsSession(max_states=4096)
    s1 = shrimp_rust.ShrimpMctsSession(max_states=4096)

...
Drives the Rust search (`ShrimpMctsSession.search`) with the four Gumbel
...
needs_rust = pytest.mark.skipif(
    shrimp_rust is None, reason="shrimp native module not built"
)
...
def test_main7_config_loads_with_gumbel_on() -> None:
    """shrimp_main_7.toml parses and enables all four Gumbel mechanisms, with
    ...
    with open(root / "configs" / "shrimp_main_7.toml", "rb") as f:
        raw = tomllib.load(f)
    cfg = parse_shrimp_config(raw["model"]["config"])
    sp = cfg.selfplay
...
def test_misplaced_policy_target_raises() -> None:
    ...
    with pytest.raises(ValueError):
        parse_shrimp_config({"selfplay": {"policy_target": "gumbel"}})
...
def test_future_opponent_policy_prefers_gumbel_target() -> None:
    ...
    from shrimp.samples import ShrimpSampleData, _future_opponent_policy

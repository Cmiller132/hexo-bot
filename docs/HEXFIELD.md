# hexfield: what it is and how it works

This is the plain-language explainer for hexfield — the neural bot at the centre
of this repository. The deep design documents
([`hexfield_blueprint.md`](hexfield_blueprint.md) for the ideas and
[`specs/hexfield_model_spec.md`](specs/hexfield_model_spec.md) for the exact
...
hexfield is an AlphaZero-style game-playing agent for Hexo, the Connect6-family
...
opinion. The network is trained entirely by self-play: hexfield plays games
...
so hexfield does not feed it a fixed-size board crop. Instead it builds a
...
`HEXFIELD_SUPPORT_RADIUS` is a knob that shrinks the support. It restricts the
...
The network is called `HexfieldNet` and lives in `model.py`. It is a graph-style
...
- `HEXFIELD_CHANNELS` — the trunk width (shipped: 192).
- `HEXFIELD_ATTENTION_HEADS` — the number of attention heads (shipped: 3, which
  puts the per-head dimension at 192/3 = 64, the size the fast attention kernels
  want).
- `HEXFIELD_TRUNK` — the block order as a string of `C` and `A` (shipped:
...
**Gumbel-Top-m root selection and Sequential Halving.** hexfield does not use
...
`hexo_train` orchestrator calling into the hexfield plugin.
...
- Board representation and features — `packages/hexfield/python/hexfield/support.py`
  and `features.py` (reference), `packages/hexfield/rust/src/support.rs` and
  `features.rs` (production); feature indices and geometry constants in
  `constants.py`.
- The network — `packages/hexfield/python/hexfield/model.py`; the env-driven
  geometry constants in `constants.py`; the fast GPU kernels in `_triton_conv.py`
  and `_triton_attn.py`.
- The search — `packages/hexfield/rust/src/search.rs` (the drivers, including
  `run_continuous`) and `tree.rs` (the PUCT tree, Gumbel, and TSS injection).
- Self-play and training targets — `packages/hexfield/python/hexfield/selfplay.py`;
  the trainer and replay window in `trainer.py`; the shard format in `shards.py`
  and `samples.py`; the losses in `losses.py`.
- The training recipe — `configs/hexfield_main_7.toml` (heavily annotated).
- Evaluation — `packages/hexfield/python/hexfield/eval_arena.py`,
  `multistage_eval.py`, and `eval_stats.py`.
- The shipped weights and their provenance — `models/MODEL_CARD.md`.

For the ideas with no code, read [`hexfield_blueprint.md`](hexfield_blueprint.md);
for the system-wide data flow, [`ARCHITECTURE.md`](ARCHITECTURE.md); for the
exact model and target contracts, [`specs/hexfield_model_spec.md`](specs/hexfield_model_spec.md).

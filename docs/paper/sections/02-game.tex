...
There are no captures or passes.  The engine has no normal draw outcome.  An
unfinished play can continue on the unbounded board, but every certificate in
this paper has a finite declared horizon.  The informal rules and the formal
model agree with the repository rule implementation
...
\begin{plainterms}
The coordinate pair names one hexagon.  The third quantity in the maximum is
the missing cube-coordinate difference.  Distance eight is therefore an
actual hex-grid radius rather than a rectangular coordinate bound.
\end{plainterms}
...
\begin{plainterms}
A winning line is always one of these six-cell sets.  A cell has six possible
positions inside a window on each of three axes.  This is why a placement
changes exactly eighteen window records.
\end{plainterms}
...
\begin{plainterms}
Window cells lie on one grid line and are never more than five steps apart.
This small diameter makes threat cells legal and supplies the locality bounds
used later.
\end{plainterms}
...
\begin{plainterms}
The state records one placement phase, not only the player whose turn it is.
This distinction is needed because the remaining budget can be one or two and
a win can stop the turn between those placements.
\end{plainterms}
...
\begin{plainterms}
Any existing stone of either colour can justify legality.  A remote defender
reply can therefore create legal cells for both players.  The soundness proof
must track that frontier even when no current threat window is touched.
\end{plainterms}
...
\begin{plainterms}
An alive window can still become a win for the named player.  A dead window
cannot, because stones are permanent.  Four stones are a threat because the
two remaining cells fit in one normal turn.
\end{plainterms}
...
\begin{plainterms}
The tactical test reduces a family of threat empties to a hitting-set
question.  A defender budget of two can answer a family with transversal
number at most two.  The LOSS result also requires the mover to lack a faster
win of its own.
\end{plainterms}
...
\begin{plainterms}
Certificate deadlines count placements, not turns.  This convention records
the exact order in which a first-stone win, a second-stone win, and the next
turn can occur.
\end{plainterms}
...
\begin{plainterms}
The paper removes defender replies only while proving an attacker win.  A
dismissal is sound when the omitted reply cannot turn that verified win into
a nonwin.  No symmetric dominance claim is intended.
\end{plainterms}

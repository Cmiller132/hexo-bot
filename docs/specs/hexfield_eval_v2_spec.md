...
Status: **design spec for the statistically-sound, multi-stage strength evaluation of the hexfield
lineage.** It is implemented in `packages/hexfield/python/hexfield/{eval_stats,eval_arena,multistage_eval}.py`;
...
the hexfield Elo scale comparable across runs and over time.
...
- **Wilson CI** on the pair-level rate (the `wilson_ci` helper in
  `hexfield/eval_stats.py`), reported per opponent.
...
eval-v2 is a **standalone orchestrator** (`hexfield/multistage_eval.py`). It **does not block the
...
- **hexfield-vs-SealBot is a concurrent round loop.** Each round, the positions where the **model**
  is to move are searched in one batched `HexfieldMctsSession.search([...keys], [...states])` call,
...
rebuild). It lives in the `hexfield` package:

| File | Contents |
|---|---|
| `docs/specs/hexfield_eval_v2_spec.md` | this spec |
| `packages/hexfield/python/hexfield/eval_stats.py` | pure-numpy statistics core: converged BT (Newton/scipy + `max|grad|<1e-6` assert + full-Hessian covariance), pair-level SE, pentanomial summary, Wilson CI, SPRT(pairs), `var_diff` difference-CI with `Cov_LB`, over-dispersion weighting, effective counts, expected-SE-Elo. **Pure CPU/numpy (+optional scipy) → unit-testable without GPU.** |
| `packages/hexfield/python/hexfield/eval_arena.py` | game-running layer: CRN-paired seat-swapped checkpoint matches and the concurrent multi-root SealBot loop. Returns the arena result-dict shape + a pentanomial block. |
| `packages/hexfield/python/hexfield/multistage_eval.py` | the multi-stage orchestrator (Stages A–D): opponent-set construction (SealBot config, permanent anchors, sliding bracket on `{5,10,20,40,80,160}`, champion registry), the rolling BT pool, and the verdict. |
| `tests/test_hexfield_eval_stats.py`, `test_hexfield_eval_arena.py`, `test_hexfield_eval_orchestrator.py` | numpy-only unit tests: BT convergence (`max|grad|<1e-6` on a known-answer ladder), pairing widens CI vs naive binomial, pentanomial variance ≤ binomial, Wilson parity, SPRT decision boundaries, difference-CI `Cov_LB` sign, no-write purely-eval sentinel. |
...
Reuse (read-only):
- `packages/hexfield/python/hexfield/evaluation.py` — the per-pair play + opening-sampling
  discipline.
- `packages/hexo_runner/python/hexo_runner/adapters/sealbot.py` (+ `_sealbot_worker.py`, hexo_runner
  match mode) — the SealBot adapter.
- `packages/hexfield/rust/src/search.rs` — the multi-root `search` API (used, not modified).
...
Pure-CPU/numpy unit tests (`tests/test_hexfield_eval_stats.py`), runnable without GPU:

...
Status: the model & lineage design spec for **hexfield** — the single neural-net lineage this
...
optional key, §5.2), so the PUCT search consumes hexfield evaluations exactly as it consumed the
...
scheduler, the performance backbone — are the inherited foundation, but hexfield is **allowed to
...
Provenance: the summary tokens are a **hexfield-designed addition**, not inherited from the
...
public test (`tests/test_hexfield_model.py`) embeds a random support into a 41×41 grid, maps the 7
...
at ratio 2, 4 heads, sdpa/materialized dual impl). The bias table below is hexfield's own design;
...
flats to support nodes. The public target tests (`tests/test_hexfield_targets.py`) pin these
...
Search is **written from scratch** in hexfield's own Rust crate (not a fork; extraction of the
...
(`hexfield._rust`, own maturin crate) — it is a self-contained crate, not a submodule of another
lineage's single-crate accelerator whose design would make every rebuild change search semantics
for all lineages at once. Building hexfield therefore never touches another lineage's `.so`.
`threats_shared` is **vendored into hexfield's own crate**
(`packages/hexfield/rust/src/threats_shared.rs`, consumed via `crate::threats_shared`) — hexfield
...
the now-retired reference search).** Originally: run hexfield search and the reference search on the
...
clipped position diverges by construction, not by bug — same seeds, hexfield in
...
(`tests/test_hexfield_search_parity.py`, `tests/test_hexfield_continuous_parity.py`) now pin
hexfield's search against its **own** `search_parity_mode` invariants rather than the retired
...
in the repo and is retained as-is in semantics. hexfield's performance work layers **around** it,
in the
...
virtual_batch_size, flush_target) mirror the shipped recipe (`configs/hexfield_main_7.toml`).
...
hexfield's search diverges from the reference lineage's baseline in a few documented ways.
...
4. **Moves-left utility — the stage-3 mechanism, adopted verbatim.** A selection-time per-edge
   ...
   hexfield's head trains on clean targets from BC prefit onward. Health: the L0 metrics
...
**Legacy read adapter:** hexfield can read existing reference-lineage compact-v1 shards by
...
window** (the on-policy buffer starts from fresh hexfield self-play only). The gold-standard variant
...
```
packages/hexfield/
  pyproject.toml                  # maturin; module hexfield._rust; deps ambient via PYTHONPATH
                                  # (torch, hexo_engine, hexo_train, ...), not pip-resolved.
                                  # Loaded by module path ([model].module = "hexfield.plugin"),
  ...
  python/hexfield/
    ...
    shards.py window.py           # hexfield_compact_v1 writer/reader + legacy adapter; replay window
...
configs/hexfield_main_7.toml      # [model] module = "hexfield.plugin"

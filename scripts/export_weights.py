"""Export an inference-only weights file from a hexfield training checkpoint.
...
Checkpoint shape (produced by hexfield.checkpoints.save_checkpoint):
    {"meta": {"lineage": "hexfield", "epoch": int, "run": str, ...},
     "model": <state_dict>,
...
The export keeps the hexfield-lineage shape so the exported file loads through
the same debug/eval loaders:
    {"meta": {..., "arch": {...}, "export": {...}}, "model": <state_dict>}

Usage:
    python scripts/export_weights.py CHECKPOINT.pt --out models/hexfield_main7_infer.pt \\
        --run hexfield_main_7 [--channels 192 --heads 3 --trunk CCACCACCACCACCA]
    python scripts/export_weights.py CHECKPOINT.pt --out OUT.pt --verify

--channels/--heads/--trunk default to the process env
(HEXFIELD_CHANNELS/HEXFIELD_ATTENTION_HEADS/HEXFIELD_TRUNK); if unset there, the
arch is inferred from the state dict itself.

--verify reloads the exported file and instantiates hexfield's model class to
prove the weights load (strict). This needs torch installed and the hexfield
package importable (run scripts/build_native.sh first, or add
packages/hexfield/python to PYTHONPATH).
"""
...
# hexfield is imported from the source tree (never pip-installed; see README).
_HF_SRC = REPO / "packages" / "hexfield" / "python"
...
def _resolve_arch(state_dict: dict, args) -> dict:
    ...
    channels = args.channels or _env_int("HEXFIELD_CHANNELS")
    heads = args.heads or _env_int("HEXFIELD_ATTENTION_HEADS")
    trunk = args.trunk or os.environ.get("HEXFIELD_TRUNK")

    if channels is None or heads is None or trunk is None:
        try:
            from hexfield.model import infer_net_kwargs_from_state_dict
            ...
            raise SystemExit(
                ...
                "Pass --channels/--heads/--trunk or set the HEXFIELD_* env vars."
            )
...
def _verify(path: Path, arch: dict) -> None:
    """Reload the export and instantiate HexfieldNet to prove the weights load."""
    ...
    try:
        ...
        raise SystemExit(
            "error: --verify needs the hexfield package importable "
            f"(and torch): {exc}\n"
            "Build it (scripts/build_native.sh) or add packages/hexfield/python "
            "to PYTHONPATH."
        )
    model = HexfieldNet(
    ...
    print(f"verify OK: HexfieldNet loaded strict, {n_params:,} params")
...
def main(argv=None) -> None:
    ...
    parser.add_argument("--channels", type=int, default=None, help="override HEXFIELD_CHANNELS")
    parser.add_argument("--heads", type=int, default=None, help="override HEXFIELD_ATTENTION_HEADS")
    parser.add_argument("--trunk", default=None, help="override HEXFIELD_TRUNK layout string")
    parser.add_argument(
        "--export-date",
    ...
    parser.add_argument(
        "--verify",
        action="store_true",
        help="reload the export and instantiate HexfieldNet to prove it loads",
    )

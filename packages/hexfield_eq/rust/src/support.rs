...
/// Model-side legal-move radius. From env var HEXFIELD_EQ_SUPPORT_RADIUS, which
...
/// Counterpart of hexfield_eq/support.py's HEXFIELD_EQ_SUPPORT_RADIUS — the
/// EQ namespace isolates it from the live hexfield's HEXFIELD_SUPPORT_RADIUS so
/// a mixed process cannot cross-configure the two featurizers.
pub fn support_radius() -> i32 {
    static R: OnceLock<i32> = OnceLock::new();
    *R.get_or_init(|| {
        std::env::var("HEXFIELD_EQ_SUPPORT_RADIUS")
            .ok()
            .and_then(|s| s.parse::<i32>().ok())
            // Out-of-range values CLAMP into [1, HALO_DIST] (matching the
            // Python readers' clamp, spec D-S26) instead of silently falling
            // back to the default, so every env value that parses agrees
            // across the three featurizer readers.
            .map(|r| r.clamp(1, HALO_DIST))

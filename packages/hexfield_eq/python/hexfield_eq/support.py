...
# Model-side support radius. HEXFIELD_EQ_SUPPORT_RADIUS restricts the support to
...
# remain at LEGAL_RADIUS. The Rust featurizer reads the same (EQ-namespaced) env
# var. Namespaced away from the live hexfield's HEXFIELD_SUPPORT_RADIUS so a
# mixed process cannot cross-configure the two featurizers' radii.
# Clamped to [1, HALO_DIST], matching the Rust support_radius() clamp (spec
# D-S26) — an out-of-range value previously diverged the two featurizers.
_SUPPORT_RADIUS = min(
    max(int(os.environ.get("HEXFIELD_EQ_SUPPORT_RADIUS", LEGAL_RADIUS)), 1),
    HALO_DIST,
)

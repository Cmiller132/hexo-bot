...
from .constants import NUM_TOKENS, RAYLEN_SLOTS, TRUNK_LAYOUT
from .samples import ExpandedRow
from .support import Support

# The (B, heads, S, S) attention-bias transient bound. 'L' layouts hold a
# larger materialized-train transient per pair (the 6-head ray bias + boolean
# live mask vs the 3-head A bias), so the default budget drops for them to
# preempt prefit OOM (spec D-S30). HEXFIELD_EQ_PAIR_BUDGET overrides either way.
_PAIR_BUDGET_ENV = os.environ.get("HEXFIELD_EQ_PAIR_BUDGET")
if _PAIR_BUDGET_ENV is not None:
    PAIR_BUDGET = float(_PAIR_BUDGET_ENV)
else:
    PAIR_BUDGET = 8.0e6 if "L" in TRUNK_LAYOUT else 2.0e7
...
def collate_rows(
    ...
    *,
    raylen: list[np.ndarray] | None = None,
) -> dict[str, torch.Tensor]:
    """Pad a list of (Support, features) rows into one model batch.

    ``raylen`` (when given) is the per-row (N, RAYLEN_SLOTS) uint8 ray-length
    array (Phase L0 wire data), positionally aligned with ``rows``; it is padded
    like the other node-major planes with pad rows 0 (a 0-length ray masks
    everything but the diagonal) and returned under the ``"raylen"`` key. ``None``
    keeps the pre-L batch key set."""
    ...
    rl = (
        torch.zeros(b, npad, RAYLEN_SLOTS, dtype=torch.uint8)
    ...
    for g, (sup, row_feats) in enumerate(rows):
        ...
        if rl is not None:
            arr = np.ascontiguousarray(raylen[g], dtype=np.uint8)
            # Fill by the array's own row count: legacy rows carry an empty
            # (0, RAYLEN_SLOTS) default and pack as zeros.
            rl[g, : arr.shape[0]] = torch.from_numpy(arr)
...
def collate_training(
    ...
    *,
    ...
    batch = collate_rows(
        [(row.support, row.feats) for row in rows],
        pad_to=pad_to,
        raylen=[row.raylen for row in rows],
    )

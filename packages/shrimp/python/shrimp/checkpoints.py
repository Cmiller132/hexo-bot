...
def save_checkpoint(path: Path, *, model: ShrimpNet, optimizer, epoch: int, extra: dict | None = None) -> Path:
    payload = {
        "meta": {"lineage": "shrimp", "epoch": int(epoch), **(extra or {})},
...
def load_into(model: ShrimpNet, payload: dict, *, optimizer=None) -> dict:
    state = payload["model"]
    ...
    if expected != got:
        ...
        raise ValueError(
            f"shrimp checkpoint key mismatch: missing={missing} unexpected={unexpected}"
        )
...
def warm_start_into(model: ShrimpNet, state: dict) -> dict:
    ...
    def load(self, checkpoint_ref, *, ctx, components) -> dict[str, Any]:
        ...
        if "meta" in payload:
            ...
            if trainer is not None:
                trainer.train_state = ShrimpTrainState.from_dict(meta.get("train_state"))

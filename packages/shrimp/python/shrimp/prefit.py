...
# SHRIMP_TRAIN_FLEX is not defaulted on for the prefit. The pair-budget
...
# path. Setting SHRIMP_TRAIN_FLEX=1 in the env forces the flex path.
...
from .losses import decode_binned_value, shrimp_loss
...
def make_optimizer(model: ShrimpNet) -> torch.optim.AdamW:
    decay, no_decay = [], []
...
def run_step(model, buckets, denoms, device, scaler, optimizer, grad_stats) -> dict:
    ...
    for batch in buckets:
        ...
        loss, components = shrimp_loss(out, batch, denominators=denoms)
    ...
    for path in shard_paths:
        ...
        for start in range(0, len(samples), NOMINAL_BATCH_ROWS):
            ...
            for bucket in pair_budget_microbuckets(expanded, quantize=1):
                ...
                out = model(batch["feats"], batch["nbr"], batch["mask"], batch["coords"])
                _, comps = shrimp_loss(out, batch, denominators=denoms)
...
def main(argv=None) -> None:
    ...
    # expect. Set SHRIMP_PREFIT_COMPILE=0 to use plain eager.
    fwd = model
    if os.environ.get("SHRIMP_PREFIT_COMPILE", "1") == "1":
        try:

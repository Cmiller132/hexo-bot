...
Exposed as the `shrimp::hex_conv` custom op (with a fake kernel), so the
serve torch.compile(dynamic=True) graph keeps it in-graph as an opaque call —
no graph breaks. Enabled via SHRIMP_TRITON_CONV=1; model.py routes to it on
...
_LN_BM = int(os.environ.get("SHRIMP_CONVLN_BM", "64"))
_LN_WARPS = int(os.environ.get("SHRIMP_CONVLN_WARPS", "4"))
_LN_STAGES = int(os.environ.get("SHRIMP_CONVLN_STAGES", "2"))
...
if HAVE_TRITON:
    ...
    @torch.library.custom_op("shrimp::hex_conv", mutates_args=())
    def hex_conv(
        x: torch.Tensor,
    ...
    @torch.library.custom_op("shrimp::hex_conv_ln", mutates_args=())
    def hex_conv_ln(
        x: torch.Tensor,
        gather_idx: torch.Tensor,
        mask: torch.Tensor,

...
Exposed as the `shrimp::attn_pair` custom op (with a fake kernel) so the
serve torch.compile graph keeps it in-graph as an opaque call. Enabled via
SHRIMP_TRITON_ATTN=1; model.py routes to it from RelPosAttention.forward on
...
Tile constants come from env (SHRIMP_ATTN_BM/BN/WARPS/STAGES, read once at
...
_BM = int(os.environ.get("SHRIMP_ATTN_BM", "64"))
_BN = int(os.environ.get("SHRIMP_ATTN_BN", "64"))
_WARPS = int(os.environ.get("SHRIMP_ATTN_WARPS", "4"))
_STAGES = int(os.environ.get("SHRIMP_ATTN_STAGES", "3"))
...
if HAVE_TRITON:
    ...
    @torch.library.custom_op("shrimp::attn_pair", mutates_args=())
    def attn_pair(
        q: torch.Tensor,
        k: torch.Tensor,
        v: torch.Tensor,

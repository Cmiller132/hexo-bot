"""shrimp training plugin: hexo_train component composition boundary."""
...
from .checkpoints import ShrimpCheckpointLoader, ShrimpCheckpointSaver
from .config import parse_shrimp_config
...
from .trainer import ShrimpTrainer
...
class ShrimpPlugin:
    ...
    def training_component_overrides(self, *, defaults, config, shared, model) -> ComponentOverrides:
        _ = (defaults, shared)
        if model is None:
            raise ValueError("ShrimpPlugin requires build_model() to run first")
        parsed = parse_shrimp_config(config)
        decay, no_decay = [], []
        ...
        trainer = ShrimpTrainer(model=model, config=parsed, optimizer=optimizer)
        return ComponentOverrides(
            trainer=trainer,
            optimizer=optimizer,
            checkpoint_loader=ShrimpCheckpointLoader(),
            checkpoint_saver=ShrimpCheckpointSaver(),

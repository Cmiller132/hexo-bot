"""shrimp_compact_v1 shard (de)serialization and the restnet adapter.
...
from .samples import STV_HORIZONS, ShrimpSampleData
...
def write_compact_shard(
    path: Path,
    samples: Sequence[ShrimpSampleData],
    *,
    short_term_value_horizons: Sequence[int] = STV_HORIZONS,
    sidecar: dict | None = None,
) -> int:
    """Serialize rows into one shrimp_compact_v1 ``.npz`` + JSON sidecar."""
...
def read_compact_shard(path: Path) -> list[ShrimpSampleData]:
    """Decode a shrimp_compact_v1 shard back into sample rows."""

    with np.load(path) as data:
        arrays = {key: data[key] for key in data.files}
    if int(arrays["schema_version"]) not in _ACCEPTED_SCHEMA_VERSIONS:
        raise ValueError(f"unsupported shrimp shard schema {int(arrays['schema_version'])}")
...
def read_legacy_restnet_shard(path: Path) -> list[ShrimpSampleData]:
    """Read a restnet compact-v1 shard as shrimp rows.

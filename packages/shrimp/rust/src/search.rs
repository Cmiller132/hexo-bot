//! shrimp search drivers: lockstep batched search and the continuous per-slot
//! scheduler.
//!
...
impl ShrimpMctsSession {
    ...
    fn search(
        ...
        if roots.len() != game_keys.len() {
            return Err(PyValueError::new_err(format!(
                "shrimp MCTS session received {} game keys for {} states",
                game_keys.len(),
                roots.len()
            )));
        }
        ...
        if roots.len() > root_limit {
            return Err(PyValueError::new_err(format!(
                "shrimp MCTS session received {} active roots, above strict limit {}",
                roots.len(),
                root_limit
            )));
        }
    ...
    fn run_continuous(
        ...
        if roots.len() != game_keys.len() {
            return Err(PyValueError::new_err(format!(
                "shrimp continuous MCTS received {} game keys for {} states",
                game_keys.len(),
                roots.len()
            )));
        }
        ...
        if roots.len() > root_limit {
            return Err(PyValueError::new_err(format!(
                "shrimp continuous MCTS received {} active roots, above strict limit {}",
                roots.len(),
                root_limit
            )));
        }
        ...
        // SHRIMP_ASYNC_EVAL: the forward is enqueued (submit, no device
        ...
        let async_eval = std::env::var("SHRIMP_ASYNC_EVAL").is_ok();
            ...
            if matches!(decision, ContinuousFlushDecision::Stop) && moves_decided == 0 {
                ...
                if moves_decided == 0 {
                    ...
                    return Err(PyRuntimeError::new_err(format!(
                        "shrimp continuous MCTS scheduler stalled with {stuck} unfinished slots \
    ...
    match item {
        ...
        ContinuousEvalItem::RootInit { state, .. } => {
            ...
            if search.root_edges_empty() {
                return Err(PyValueError::new_err(
                    "shrimp continuous MCTS root has no legal actions",
                ));
            }

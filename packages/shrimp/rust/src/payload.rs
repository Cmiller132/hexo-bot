...
fn parse_chunk_reply(
    ...
    let values_obj = output
        ...
        .map_err(|_| PyValueError::new_err("shrimp evaluator output missing values_bytes"))?;
    let priors_obj = output
        .get_item("priors_bytes")
        .map_err(|_| PyValueError::new_err("shrimp evaluator output missing priors_bytes"))?;
    ...
    let moves_left_bytes: Option<Vec<u8>> = if request_moves_left {
        let obj = output.get_item("moves_left_bytes").map_err(|_| {
            PyValueError::new_err(
                "shrimp evaluator output missing moves_left_bytes (request_moves_left was set)",
            )
        })?;
...
pub fn evaluate_state_refs_cached(
    ...
    Ok(result_slots
        .into_iter()
        .map(|item| item.expect("every shrimp evaluation slot must be populated"))
        .collect())
}
